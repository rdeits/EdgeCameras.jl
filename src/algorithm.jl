...
function reconstruct(cam::EdgeCamera, time_range::Tuple, target_rate=framerate(cam.source))
    ...
    # enumerateframes(cam.source.video, times) do i, buffer
    ...
    #     end
    # end
    # buffer = zeros(RGB{N0f8}, cam.source.video.height, cam.source.video.width)
    buffer::FRAME_TYPE = read(cam.source.video)
    for (i, current_time) in enumerate(times)
        seek(cam.source.video, convert(Float64, current_time / (1u"s")))
        read!(cam.source.video, buffer)
        sample!(pixels, cam, buffer, cam.params.blur)
        pixels .-= background_samples
        for k in 1:length(pixels)
            for j in 1:size(data, 1)
